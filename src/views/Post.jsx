...
import { View, Text, TextInput, ScrollView, Image, StyleSheet, SafeAreaView, StatusBar, Pressable, } from 'react-native';
import { useNavigation } from '@react-navigation/native';
import { Picker } from '@react-native-picker/picker';
import Icon from 'react-native-vector-icons/FontAwesome';
import { useLoading } from '../components/Loading';
import { getPostPage } from '../utils/api'
import { UTF8ToGBK } from '../utils/index'
...
const PostScreen = ({ route }) => {
  ...
  const handlePublish = () => {
    ...
    fd.append('subject', UTF8ToGBK(subject))
    fd.append('selecttypeid', selectTypeId)
    fd.append('message', UTF8ToGBK(message))
    fd.append('readperm', readPermission)
    Object.keys(extraParamValues).forEach(key => {
      fd.append(key, extraParamValues[key])
    })
    http.post('post.php?action=newthread&fid=230&extra=page%3D1&topicsubmit=yes', fd).then(res => console.log(res))
  };
